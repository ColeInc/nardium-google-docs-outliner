...
export const fetchFileContents = async (
    ...
    const checkErrorCount = async () => {
        ...
        if (retryCount >= 2) {
            ...
            await fetchNewAccessToken.current();
    ...
    const mockData = {
        ...
        // };
        // documentCtx.updateDocumentDetails({ documentContent: contents } );
        // return Promise.resolve(contents);
    }

    try {
        console.log("fetchFileContents: sending request w/ these:", { isLoggedIn, token, documentId, retryCount });
